...
<div class="row">
    <div class="col-xl-12">
        <div class="card m-b-30">
            <div class="card-body">
                <form action="{{route('dashboard.patient-records.update',$result->id)}}" method="post" autocomplete="off" enctype="multipart/form-data" id="frmStore">
                    @method("PUT")
                    @csrf
                    <div class="row mb-3">
                        <div class="col-lg-12">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-10">
                                    <select class="form-control select2" name="doctor_id" required>
                                        ...
                                        <option value="{{$row->id}}" @selected($row->id == $result->doctor_id)>{{$row->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-md-10">
                                    <input type="text" class="form-control" name="name" placeholder="Nama Pasien" value="{{$result->name}}" required>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-md-2 col-form-label">NIK <span class="text-danger">*</span></label>
                                <div class="col-md-10">
                                    <input type="number" class="form-control" name="nik" placeholder="NIK" value="{{$result->nik}}" required>
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-md-10">
                                    <select class="form-control" name="gender" required>
                                        ...
                                        <option value="L" @selected($result->gender == "L")>Laki-Laki</option>
                                        <option value="P" @selected($result->gender == "P")>Perempuan</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-md-10">
                                    <input type="date" class="form-control" name="date_of_birth" placeholder="Tanggal Lahir" value="{{$result->date_of_birth}}" required>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-md-2 col-form-label">Tinggi Badan (cm)</label>
                                <div class="col-md-10">
                                    <input type="number" class="form-control" name="height" placeholder="Tinggi Badan (cm)" value="{{$result->height}}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-md-2 col-form-label">Berat Badan (kg)</label>
                                <div class="col-md-10">
                                    <input type="number" class="form-control" name="weight" placeholder="Berat Badan (cm)" value="{{$result->weight}}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-md-2 col-form-label">Systole</label>
                                <div class="col-md-10">
                                    <input type="number" class="form-control" name="systole" placeholder="Systole" value="{{$result->systole}}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-md-2 col-form-label">Diastole</label>
                                <div class="col-md-10">
                                    <input type="number" class="form-control" name="diastole" placeholder="Diastole" value="{{$result->diastole}}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-md-2 col-form-label">Heart Rate</label>
                                <div class="col-md-10">
                                    <input type="number" class="form-control" name="heart_rate" placeholder="Heart Rate" value="{{$result->heart_rate}}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-md-2 col-form-label">Respiration Rate</label>
                                <div class="col-md-10">
                                    <input type="number" class="form-control" name="respiration_rate" placeholder="Respiration Rate" value="{{$result->respiration_rate}}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-md-2 col-form-label">Suhu Tubuh</label>
                                <div class="col-md-10">
                                    <input type="number" class="form-control" name="temperature" placeholder="Suhu Tubuh" value="{{$result->temperature}}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-md-2 col-form-label">Keterangan</label>
                                <div class="col-md-10">
                                    <textarea class="form-control" name="note" placeholder="Keterangan">{{$result->note}}</textarea>
                                </div>
                            </div>
                            <div class="form-group row">
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody class="tbody-repeater">
                                        @forelse($result->prescriptions as $index => $row)
                                        <tr class="repeater">
                                            <input type="hidden" class="form-control medicine_name" name="repeater[{{$index}}][medicine_name]" value="{{$row->medicine_name}}">
                                            <input type="hidden" class="form-control price" name="repeater[{{$index}}][price]" value="{{$row->price}}">
                                            <td class="number">{{$index+1}}</td>
                                            <td>
                                                <select class="form-control select-medicine medicine_id" name="repeater[{{$index}}][medicine_id]">
                                                    <option value="">==Pilih Obat==</option>
                                                    <option value="{{$row->medicine_id}}" selected>{{$row->medicine_name}}</option>
                                                </select>
                                            </td>
                                            <td>
                                                <input type="text" class="form-control price-readonly" value="{{number_format($row->price,0,',','.')}}" readonly>
                                            </td>
                                            <td>
                                                <input type="number" class="form-control qty" name="repeater[{{$index}}][qty]" value="{{$row->qty}}">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control note" name="repeater[{{$index}}][note]" value="{{$row->note}}">
                                            </td>
                                            <td>
                                                @if($index >= 1)
